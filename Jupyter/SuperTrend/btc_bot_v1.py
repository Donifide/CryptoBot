...
in_position = False
bar = exchange.fetch_ohlcv('BTC/USD', timeframe='1m', limit=5)

def check_buy_sell_signals(df):
    global in_position, order_size
    ...
    if not df['in_uptrend'][previous_row_index] and df['in_uptrend'][last_row_index]:
        ...
        if not in_position:
            order = exchange.create_market_buy_order('BTC/USD',order_size)
    ...
    if df['in_uptrend'][previous_row_index] and not df['in_uptrend'][last_row_index]:
        if in_position:
            ...
            order = exchange.create_market_sell_order('BTC/USD',order_size)
            print('Status:'+order['info']['status'],
                  'Price:'+order['trades'][0]['info']['price'],
                  'Quantity:'+order['info']['executedQty'],
                  'Type:'+order['info']['side'])
